...
class ShowPlayertraderPage extends AbstractGamePage
{
    ...
    public function tradeResources()
    {
        ...
        if($trade == TRUE) {
            ...
            $course              = $this->getAverageCourse();
            $aktCourse           = $course[0][$sellerResource . $sellerWantsResource];
            ...
            $newOnBuyerResource1 = $planeteBuyerResources[0][$tradeData["changeRes"]] - $sendResources;
            $newOnBuyerResource2 = $planeteBuyerResources[0][$tradeData["resType"]] + $buy;

            $vals = [$tradeData["changeRes"] . ' = ' . $newOnBuyerResource1, $tradeData["resType"] . ' = ' . $newOnBuyerResource2, 'last_update = ' . time()]; // $tradeData["resType"].' = '.$newOnBuyerResource2,

            #echo 'before: ' . $planeteBuyerResources[0][$tradeData["resType"]] . ' buy: ' . $buy . ' after: ' . $newOnBuyerResource2."<br />";
            #echo 'kaufe: '. $buy. ' '. $sellerResource . " " . " Kurs: " . $aktCourse . " gib: " . $sendResources." " . $sellerWantsResource." ab<br />";
            #echo "gekauft Res: ". $buy . ' ' . $sellerResource .  " neu auf Plani: " . $newOnBuyerResource2."<br />";
            /*echo "Res auf Plani vorher:<br />";
            echo '<pre>';
            print_r($planeteBuyerResources);
            echo '</pre>';
            echo "gekauft Res: ". $buy . ' ' . $sellerResource .  " neu auf Plani: " . $newOnBuyerResource2."<br />";
            echo "abgegeben Res: ". $sendResources . ' ' . $sellerWantsResource. ' neu auf Plani: ' . $newOnBuyerResource1."<br />";*/
