...
class ShowRegisterPage extends AbstractLoginPage
{
    ...
    function show()
    {
        ...
        $this->assign([
                          ...
                          'pageStarttime'     => base64_encode(time()),
                      ]);
    ...
    function send()
    {
        global $LNG;
        $config                     = Config::get();
        $botMailAdress              = HTTP::_GP('e-mail', '');
        $PageStartTime              = base64_decode(HTTP::_GP('musculusMaximus', ''));
        $PageSubmitTime             = time();
        $registrationFailureDueTime = FALSE;
        
        if($PageSubmitTime - $PageStartTime < 5 || $PageSubmitTime - $PageStartTime > 60){
            $registrationFailureDueTime = TRUE;
        }
        ...
        if($botMailAdress != "" || $registrationFailureDueTime){
            #@mail('Silvio.Habel@gmail.com', "Space-Tactics Registration Bot detected", "Bot detected on ".date("d.m.Y H:i:s")." from ".$_SERVER['REMOTE_ADDR']."\n\n".print_r($_POST, TRUE));
            @error_log('Registration Bot detected on '.date("d.m.Y H:i:s").' from '.$_SERVER['REMOTE_ADDR']."\n\n".print_r($_POST, TRUE), 3);
